...
def gen_conv(x, cnum, ksize, stride=1, rate=1, name='conv',
    ...
    x = tf.layers.conv2d(
        ...
        activation=None, padding=padding, name=name, reuse=tf.AUTO_REUSE)
...
def compute_color(u,v):
    ...
    for i in range(np.size(colorwheel,1)):
        ...
        col[notidx] *= 0.75
        img[:, :, i] = np.float32(np.floor(255 * col*(1-nanIdx)))
    return img
