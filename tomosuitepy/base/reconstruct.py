...
def obtain_rot_center_sinos(prj, shift_range=20):
    ...
    for idx, array in enumerate(return_shifted):
        if idx < 1:
            ...
            storing = np.append(storing, np.reshape(
                array[:, 0, :], new_shape), axis=1)
...
def reconstruct_single_slice(prj_data, theta, rows=(604, 606),
    ...
    if recon_type == 'standard':
        ...
        if rot_center_shift_check is not None:
            prj, rot_center, og_center, range_array = obtain_rot_center_sinos(
                prj, rot_center_shift_check)
    ...
    if edge_transition is not None:
        ...
        print(
            f"Applying Sinogram Edge Crop. New Rotation Center is - {rot_center}")
    ...
    if chunker_save:
        ...
        for it in range(0, idx+1):
            os.remove(
                f"{basedir}/tomsuitepy_recon_save_it_{str(it).zfill(4)}.tiff")
...
def reconstruct_data(basedir,
    ...
    if network == None:
        ...
        start, end, prj_data, theta = prepare_deepfillv2(
            basedir, checkpoint_num, start_row, end_row, verbose=verbose)

    elif network == 'rife':
        recon_type = 'standard'
        start, end, prj_data, theta = prepare_rife(
            basedir, start_row, end_row, rife_types=rife_types, verbose=verbose)

    if chunk_recon_size > 1:
        p_cwd = pathlib.Path('.').absolute()
        print(
            f'Temporary files to be saved to {p_cwd} - Directory Real - {os.path.isdir(p_cwd)}')
    ...
    try:
        ...
        raise ValueError(
            f"Failed to initiate muppy RAM collection - Error: {ex}")

    if select_prjs2use is not None:
        prj_data = prj_data[tuple([select_prjs2use])]
        theta = theta[tuple([select_prjs2use])]
        print(
            f'Adjusting prj amount to {len(select_prjs2use)} Prj From Recon. len_prj_data={len(prj_data)}  len_theta={len(theta)}')
...
def plot_reconstruction_centers(slc_proj, figsize=(15, 15), clim=(None, None), cmap='Greys_r',
    ...
    if starting_rotation_center is None:
        ...
        center_range_values = np.arange(
            starting, starting_rotation_center + total)

        if interactive:

            sliders = widgets.IntSlider(
                value=starting, min=starting, max=starting_rotation_center + total - 1)

            interact(plotting_center, slcs_proj=fixed(slc_proj),
                     center_range_values=fixed(center_range_values),
                     idx=sliders, starting_rotation_center=fixed(
                         starting_rotation_center),
...
def plotting_center(slcs_proj, center_range_values, idx, starting_rotation_center, figsize, cmap, clim):
    ...
    if starting_rotation_center is None:
        ...
        plt.title(
            f'Row Num: {row}, Mean: {np.mean(prj)}, Rotation Center: {starting}')
        starting += 1
