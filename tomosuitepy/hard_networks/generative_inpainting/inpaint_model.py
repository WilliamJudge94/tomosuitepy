...
from inpaint_ops import random_bbox, bbox2mask, local_patch, brush_stroke_mask, determing_mask_values
...
class InpaintCAModel(Model):
    ...
    def build_graph_with_losses(
        ...
        #with tf.variable_scope('mask'), tf.device('/cpu:0'):
        #    loading = np.load('/local/data/wjudge/TomoSuite2/v4/deepfillv2/testing.npy')
        #    loading += 1
        #    np.save('/local/data/wjudge/TomoSuite2/v4/deepfillv2/testing.npy', loading)
        
        determing_mask_values(FLAGS, FLAGS.mask_type, udr=FLAGS.udr)
        ...
        #mask = select_mask(FLAGS, FLAGS.mask_type, udr=FLAGS.udr)
    ...
    def build_infer_graph(self, FLAGS, batch_data, bbox=None, name='val'):
        ...
        if FLAGS.udr == 'r':
            if int(FLAGS.static_fnames[0].split('/')[-1][:-4]) % 2 == 0:
                udr = 'u'
            else:
                udr = 'd'
        else:
        ...
        #mask = select_mask(FLAGS, FLAGS.mask_type, udr=udr)
...
def select_mask(FLAGS, mask_type, udr='r'):
    ...
    if mask_type == 'edge':
        ...
        if udr == "u":
            ...
            start_position = master_shape - half_master_height - 1

        elif udr == 'r':
            if FLAGS.random_value % 2 == 0:
                start_position = 0
            else:
                start_position = master_shape - half_master_height - 1

            FLAGS.random_value += random.randint(1, 2)

            # Stop the random number from reaching a very large number
            if FLAGS.random_value >= random.randint(1000, 2000):
                FLAGS.random_value = 0 
            ...
            #print('/n')
            #print(f'Staring Position {start_position} -- {FLAGS.random_value}')
            #print('/n')

            #print(f'Random {random_value} {start_position}')
                
                
        bbox = (tf.constant(start_position), tf.constant(0),
            tf.constant(half_master_height), tf.constant(master_shape))
        ...
        bbox = (tf.constant(start_position), tf.constant(0),
                    tf.constant(master_height), tf.constant(master_shape))
