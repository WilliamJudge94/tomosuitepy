...
def train_tomogan(basedir, mb_size=2, img_size=896, types='noise', epochs=120001, gpus='0', lmse=0.5, lperc=2.0,
    ...
    generator = make_generator_model(input_shape=(
        None, None, in_depth), nlayers=int(lunet))
    ...
    for epoch in range(epochs):
        ...
        for _ge in range(gene_iters):
            ...
            with tf.GradientTape() as gen_tape:
                ...
                vggf_gt = feature_extractor_vgg.predict(
                    tf.concat([y_mb, y_mb, y_mb], 3).numpy())
                vggf_gen = feature_extractor_vgg.predict(
                    tf.concat([gen_imgs, gen_imgs, gen_imgs], 3).numpy())
                perc_loss = tf.losses.mean_squared_error(
                    vggf_gt.reshape(-1), vggf_gen.reshape(-1))
        ...
        if epoch % (200//gene_iters) == 0:
            X222, y222 = get1batch4test(
                x_fn=xtest, y_fn=ytest, in_depth=in_depth)
            ...
            with summary_writer.as_default(), tf.contrib.summary.always_record_summaries():
                tf.contrib.summary.scalar(
                    'gen_loss', tf.constant(gen_loss), step=epoch)
                tf.contrib.summary.scalar(
                    'gen_loss-mse_lambda-mse', tf.constant(loss_mse*lambda_mse), step=epoch)
                tf.contrib.summary.scalar(
                    'gen_loss-adv_lambda-adv', tf.constant(loss_adv*lambda_adv), step=epoch)
                tf.contrib.summary.scalar(
                    'gen_perc-loss_lambda-perc', tf.constant(perc_loss*lambda_perc), step=epoch)

                tf.contrib.summary.scalar(
                    'disc_loss', tf.constant(disc_loss), step=epoch)
                tf.contrib.summary.scalar('disc_real_o', tf.constant(
                    disc_real_o.numpy().mean()), step=epoch)
                tf.contrib.summary.scalar('disc_fake_o', tf.constant(
                    disc_fake_o.numpy().mean()), step=epoch)
                ...
                if epoch == 0:
                    ...
                    tf.contrib.summary.image('gtruth', tf.constant(
                        ground_truth_image), step=epoch, max_images=1)
                    tf.contrib.summary.image('noisy', tf.constant(
                        noisy_image), step=epoch, max_images=1)
                ...
                tf.contrib.summary.image('predictions', tf.constant(
                    predicted_image), step=epoch, max_images=1)
