...
class N2NCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        with self.summary_writer.as_default(), tf.contrib.summary.always_record_summaries():
            tf.contrib.summary.image('input', tf.constant(image_og), step=epoch, max_images=15)
            tf.contrib.summary.image('prediction', tf.constant(prediction), step=epoch, max_images=15)
