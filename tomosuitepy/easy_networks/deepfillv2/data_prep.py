...
def zero_prj4missing_wedge(prj_data, number2zero):
    ...
    if number2zero > determine_max_number2zero_value(prj_data):
        delta_width_change, half_delta_width_change = determine_delta_sinogram_width(
            prj_data, number2zero)
...
def normalize_sinograms(prj_sinograms):
    prj_sinograms = prj_sinograms / \
        np.max(prj_sinograms, axis=(1, 2), keepdims=True)
    prj_sinograms *= 255.0
...
def downscale_base_testing_images(base_testing_images, new_shape):
    ...
    reduce_dimensions_data = np.reshape(
        base_testing_images, (first_shape, shape[2], shape[3]))
    ...
    for train_image in tqdm(training_images, desc='Saving Training Data'):
        ...
        if ud_flip:
            ...
            if lr_flip:
                ...
                image_ud_lr = np.dstack(
                    (ud_lr_image, ud_lr_image, ud_lr_image))
...
def format_and_save_data4deepfillv2(basedir, number2zero, types='base', downscale_shape=(512, 512),
    ...
    number_of_training_images = determine_how_many_sinogram_training_images(
        prj_sinograms, number2zero)
    base_images = obtain_base_images(prj_sinograms, number2zero)

    base_training_images = obtain_base_training_images(
        prj_sinograms, number2zero, number_of_training_images)
    base_testing_images = obtain_base_testing_images(
        prj_sinograms, number2zero, number_of_training_images)
    ...
    downscaled_base_training_images = downscale_base_training_images(
        base_training_images, downscale_shape)

    if edge_or_central == 'central':
        downscaled_central_testing_images = downscale_central_testing_images(
            central_testing_images, downscale_shape)
        save_training_images(basedir, downscaled_base_training_images,
                             downscaled_central_testing_images, lr_flip, ud_flip)

    elif edge_or_central == 'edge':
        downscaled_base_testing_images = downscale_base_testing_images(
            base_testing_images, downscale_shape)
        save_training_images(basedir, downscaled_base_training_images,
                             downscaled_base_testing_images, lr_flip, ud_flip,  val_ud_flip=val_ud_flip)
...
def convert2gray_rescale_save(predicted_images, basedir, number2zero, checkpoint_num,
    ...
    upscaler_shape = (
        base_training_images.shape[1], base_training_images.shape[2])
    ...
    inverted_images = invert_central_prediction_images(
        base_images, gray_upscaled_images*255.0, number2zero)
